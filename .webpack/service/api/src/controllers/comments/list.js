...
async function main(event, context, callback) {
  ...
  let comments;
  try {
    comments = await _comment2.default.findAll({
    ...
    const response = {
      statusCode: 500,
      body: (0, _stringify2.default)({ error: 'Unable to retrieve Comments' })
    };
    callback(null, response);
    return;
  }
  ...
  const response = {
    ...
    body: (0, _stringify2.default)({ status: 'success', comments })
  };
  callback(null, response);
}
...
const Comment = _consts.sequelize.define('Comment', {
  id: {
    allowNull: false,
    autoIncrement: true,
    primaryKey: true,
