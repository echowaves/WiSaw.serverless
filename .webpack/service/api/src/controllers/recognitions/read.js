...
var _recognition = __webpack_require__(26);
...
async function main(event, context, callback) {
  ...
  let recognition;
  try {
    recognition = await _recognition2.default.findOne({
      where: { photoId: id }
    });
    ...
    const response = {
      statusCode: 500,
      body: (0, _stringify2.default)({ error: 'Unable to retrieve recognition' })
    };
    callback(null, response);
    return;
  }
  ...
  const response = {
    ...
    body: (0, _stringify2.default)({ status: 'success', recognition })
  };
  callback(null, response);
}
...
class Recognition extends _sequelize.Model {}
