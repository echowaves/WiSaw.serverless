...
async function main(event, context, cb) {
  if (process.env.STAGE !== 'prod') {
    ...
    const response = {
      statusCode: 400,
      body: (0, _stringify2.default)({ error: 'wrong environment' })
    };
    cb(null, response);
    return false;
  }
  ...
  try {
    ...
    const response = {
      statusCode: 500,
      body: (0, _stringify2.default)({ error: 'Unable to retrieve Photos sitemap' })
    };
    cb(null, response);
    return false;
  }

  const sitemap = _sitemap2.default.createSitemap({
  ...
  photos.forEach(photo => {
    const jsonObj = JSON.parse((0, _stringify2.default)(photo));
...
/***/ })
/******/ ])));
//# sourceMappingURL=createSiteMap.js.map