...
var _pgHstore = __webpack_require__(5);

var _pgHstore2 = _interopRequireDefault(_pgHstore);

var _lambdaLog = __webpack_require__(6);
...
const devConfig = __webpack_require__(8).config();
const testConfig = __webpack_require__(9).config();
const prodConfig = __webpack_require__(10).config();
...
var _abuseReport = __webpack_require__(17);
...
const AWS = __webpack_require__(13);
...
async function main(event, context, callback) {
  ...
  const c = await _abuseReport2.default.count({ where: { uuid } });
  ...
  const createdAt = (0, _moment2.default)();
  ...
  let photo;
  try {
    photo = await _photo2.default.create({
      uuid,
      location,
      likes,
      createdAt,
  ...
  let watcher;
  try {
    watcher = await _watcher2.default.create({
      uuid,
      photoId: photo.id,
      createdAt,
      updatedAt,
...
const AbuseReport = _consts.sequelize.define('AbuseReport', {
  id: {
    allowNull: false,
    autoIncrement: true,
    primaryKey: true,
