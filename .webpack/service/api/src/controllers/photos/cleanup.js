...
async function main(event, context, callback) {
  ...
  // cleanup photos
  try {
    await _consts.sequelize.query('DELETE FROM "AbuseReports" where "createdAt" < NOW() - INTERVAL \'7 days\'');
    ...
    // }
    count = await _consts.sequelize.query('select count(*) FROM "Photos"');
  } catch (err) {
    console.log('Unable to cleanup Photos', err);
    const response = {
      statusCode: 500,
      body: (0, _stringify2.default)({ error: 'Unable to cleanup Photos', err })
    };
